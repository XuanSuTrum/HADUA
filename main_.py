...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer):
    ...
    ema_log = []  # debug-only: verifies that EMA state persists across batches
    for e in range(args.n_epoch):
        ...
        np.savetxt(r'F:\Emotion_datasets\SEED\train_log.csv', np_log, delimiter=',', fmt='%.6f')

        stats = model.match_weighting.get_stats(current_epoch=e)
        class_dist = stats['class_dist']
        np.savetxt(
            r'F:\Emotion_datasets\SEED\ema_stats_log.csv',
            delimiter=',',
            fmt='%.8f',
            header='epoch,mu,var,sigma,alpha,' + ','.join(
                [f'class_dist_{i}' for i in range(args.n_class)]
            ),
            comments=''
        )
    ...
    print('Final epoch result: Acc: {:.4f}, Precision: {:.4f}, F1: {:.4f}, AUC: {:.4f}'.format(
        last_acc, last_precision, last_f1, last_auc))
    print('Confusion Matrix:\n', last_confusion_matrix)
    return last_acc, last_confusion_matrix, last_precision, last_f1, last_auc
